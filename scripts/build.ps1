...
Try {
    ...
    If ($BuildProject) {
        If ($NDebug) {
            Step-VisualStudio $MsBuild "$RootFolder\g.shift.sln" -ThrowOnError $True -Configuration "Debug" -WarningLevel "3"
        }

        If ($NRelease) {
            Step-VisualStudio $MsBuild "$RootFolder\g.shift.sln" -ThrowOnError $True -Configuration "Release" -WarningLevel "3"
        }
    }
